...
class TraceHelper():
    ...
    def __init__(self, abstraction_path: str, abstraction_exact_match: bool, abstraction_delimiter: str, verbose: bool, remove_unlisted: bool, anamnesis_events: str):
    ...
    def translate_procedure_diagnosis_material_to_event(self, event) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
        """
    ...
    def create_trace_information(self, event) -> dict:
        """
    ...
    def deduplicate_relevant_events(self, relevant_events: List[dict]) -> List[dict]:
    ...
    def create_XEvent(self, event: dict) -> XEvent:
    ...
    def create_XTrace(self, trace_events: list, trace_type: str, trace_information: dict) -> XTrace:
    ...
    def create_event_dict(self, event, event_name: str, event_descriptor: Optional[str],
    ...
    def identify_duplicate_medication_diagnosis_events(self, trace_events: list) -> Tuple[list, dict, dict, dict]:
    ...
    def apply_event_filters(self, trace_events: list, event_filter: Filter, seen_running_medications_per_day: dict,
                            seen_end_medications_per_day: dict, seen_diagnosis_per_day: dict) -> Tuple[List[dict], set]:
        medication_list = []
        encounter_ids = set()
        ...
        for event in trace_events:
            ...
            if event_name is not None:
                ...
                event = self.create_event_dict(event, event_name, event_descriptor,
                                               event_context, event_code, lifecycle_state, new_timestamp)
                relevant_events.append(event)
                encounter_ids.add(event['visit_id'])

        return relevant_events, encounter_ids

    def create_XES_trace_based_on_trace_type(self, trace_type, relevant_events, trace_information) -> List[XTrace]:
        ...
        if trace_type == 'visit':
            ...
            for trace_id in encounter_traces.keys():
                xes_traces.append(self.create_XTrace_for_trace_type(trace_information,
                                                                    encounter_traces[trace_id],
                                                                    trace_type))
        else:
            xes_traces.append(self.create_XTrace_for_trace_type(
                trace_information, relevant_events, trace_type))

        return xes_traces

    def create_XTrace_for_trace_type(self, trace_information: dict, trace_events: list, trace_type: str) -> XTrace:

        trace = self.create_XTrace(trace_events=trace_events,
                                   trace_information=trace_information, trace_type=trace_type)

        for event in trace_events:
            # Create opyenxes event and append it to the trace
            log_event = self.create_XEvent(event)
