...
def create_XEvent(event: dict) -> XEvent:
...
def create_XTrace(trace_events: list, trace_type: str, trace_information: dict) -> XTrace:
...
def create_event_dict(event, event_name, event_descriptor,
                      event_context, event_code,  lifecycle_state, new_timestamp) -> dict:
    event_dict = {
        "mrn": event.medical_record_number,
...
def identify_duplicate_medication_diagnosis_events(trace_events: list, abstraction_path: str,
                                                   abstraction_exact_match: bool, abstraction_delimiter: str,
                                                   verbose: bool, remove_unlisted: bool, anamnesis_events: str) -> Tuple[list, dict, dict, dict]:
...
def create_XES_trace_based_on_trace_type(trace_type, relevant_events, trace_information) -> List[XTrace]:
    ...
    if trace_type == 'visit':
        ...
        for trace_id in encounter_traces.keys():
            xes_traces.append(create_XTrace_for_trace_type(trace_information,
                                                           encounter_traces[trace_id],
                                                           trace_type))
    else:
        xes_traces.append(create_XTrace_for_trace_type(
            trace_information, relevant_events, trace_type))

    return xes_traces


def create_XTrace_for_trace_type(trace_information: dict, trace_events: list, trace_type: str) -> XTrace:

    trace = create_XTrace(trace_events=trace_events,
                          trace_information=trace_information, trace_type=trace_type)

    for event in trace_events:
        # Create opyenxes event and append it to the trace
        log_event = create_XEvent(event)
